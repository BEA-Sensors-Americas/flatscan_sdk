...
from frontend.sensor_params import SensorParams
...
class Renderer:
    ...
    def reverse_rescale_linear(self, value):
        # Reverse the rescaling applied to dist
        original_value = 2.0 * value  # Assuming 0.5 * self.clip(a, new_min, new_max) was used
        return original_value

    def reverse_clip(self, t, new_min, new_max):
        if t > 200:
            original_t = (t - 0) / 0.2
        else:
            original_t = (t - 0) / 0.1
...
class DataPollingThread(QThread):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.timeStart = datetime.now().strftime("%Y-%m-%d-%H%M")
        self.fileName = f"data_" + self.timeStart + ".csv"
        self.csv_file = open(self.fileName, 'w', newline='')
        self.csv_writer = csv.writer(self.csv_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        self.param = SensorParams();
