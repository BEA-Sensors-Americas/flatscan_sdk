...
import device_type.piano_device as fs
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.zoneDict = {1: self.c4, 2: self.d4, 3: self.e4, 4: self.f4, 5: self.g4, 6: self.a4, 7: self.b4, 8: self.c5,
                         9: self.d5, 10: self.e5, 11: self.f5, 12: self.g5, 13: self.a5, 14: self.b5, 15: self.c6}
    ...
    def run_threads(self):
        self.th[self.mw.sender().objectName()] = Thread(target=self.play_notes, args=(
    ...
    def playMDI(self, mdi):
        for i in range(len(mdi)):
            if mdi[i] > 8:
                self.zoneDict[i].click()
        time.sleep(0.2)

    def startMDIThread(self):
        self.mdiThread = FlatScanThread()
        self.connect(self.mdiThread, QtCore.SIGNAL("playMDI(PyQt_PyObject)"), self.playMDI)
        self.mdiThread.start()
...
class FlatScanThread(QtCore.QThread):
    ...
    def run(self):
        while (1):
            mdi = self.fs.print_mdi()
            self.emit(QtCore.SIGNAL("playMDI(PyQt_PyObject)"), mdi)
        time.sleep(0.2)
